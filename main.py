...
def get_sparisity(net, solution):
    ...
    for n, m in net.named_modules():
        if hasattr(m, 'prune_rate'):
            ...
            sparsity.append(args.prune_rate + 0.05 - sparse)
            k+=1
    return sparsity

def fitness_function(solution, net, val_loader, train_loader, logger, search = True):
...
def genetic_algorithm(net, logger,val_loader, train_dataset):
    ...
    for generation in range(generations):
        ...
        fitness_values = [fitness_function(solution, net, val_loader, calib_loader, logger) for solution in population]

        parents = selection(population, fitness_values, num_parents)

        cross_childs = crossover(parents, crossover_rate, vector_length)
        mutation_childs = mutation(parents, mutation_rate)
        random_num = population_size - len(cross_childs) - len(mutation_childs)
    ...
    best_solution = max(population, key=lambda solution: fitness_function(solution, net, val_loader, 
                                                                            calib_loader, logger))
...
def main_worker(args):
    ...
    if 'ImageNet' in args.set:
        for key in list(ckpt.keys()):
            ...
            elif 'fc.bias' in key or 'classifier.bias' in key:
                _ = ckpt.pop(key)
            else:
    ...
    best_solution = [0.0, 0.5773847866671289, 0.5773847866671289, 0.5773847866671289, 0.5773847866671289, 0.6877693573137744, 0.7934231606469921, 0.0, 0.7934231606469921, 0.7934231606469921, 0.846250062313601, 0.897894272151853, 0.0, 0.897894272151853, 0.897894272151853, 0.923716377070979, 0.9492428090330157, 0.3169954691238813, 0.9492428090330157, 0.9492428090330157, 0.656205677798585]
    k = 0
    for n, m in net.named_modules():
        if hasattr(m, 'prune_rate'):
